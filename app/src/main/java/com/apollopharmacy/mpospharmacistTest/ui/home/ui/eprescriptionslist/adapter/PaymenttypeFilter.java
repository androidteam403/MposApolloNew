...
import com.apollopharmacy.mpospharmacistTest.databinding.ViewPaymenttypeFilterBinding;
...
public class PaymenttypeFilter extends RecyclerView.Adapter<PaymenttypeFilter.ItemBaseViewHolder> implements Filterable
{
    public static final int ITEM_TYPE_ACTION_WIDTH = 1001;
    private List<PaymenttypeModel> mOrderArrList;
    private List<PaymenttypeModel> mFilteredOrderArrList;
    ...
    public PaymenttypeFilter(Context context, ArrayList<PaymenttypeModel> orderInfoArrList, EprescriptionsListMvpPresenter<EprescriptionsListMvpView> presenter) {
    ...
    @NotNull
    @Override
    public PaymenttypeFilter.ItemBaseViewHolder onCreateViewHolder(@NotNull ViewGroup parent, int viewType) {
        ViewPaymenttypeFilterBinding itemMainBinding = DataBindingUtil.inflate(LayoutInflater.from(parent.getContext()), R.layout.view_paymenttype_filter, parent, false);

        return new PaymenttypeFilter.ItemBaseViewHolder(itemMainBinding);
    }

    @Override
    public void onBindViewHolder(@NotNull final PaymenttypeFilter.ItemBaseViewHolder holder, int position) {
        // OMSTransactionHeaderResModel.OMSHeaderObj item = mFilteredOrderArrList.get(position);
        PaymenttypeModel paymenttypeModel= mFilteredOrderArrList.get(position);

        holder.listItemMainBinding.customertype.setText(paymenttypeModel.getPaymenttype());
        holder.listItemMainBinding.statusview.setTag(position);
        Integer pos = (Integer) holder.listItemMainBinding.statusview.getTag();
        if(Constant.getInstance().paymenttypearraylist.contains(paymenttypeModel.getPaymenttype()))
        {
        ...
        holder.listItemMainBinding.statusview.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (mFilteredOrderArrList.get(pos).getCheckstatus()) {
                    ...
                    if(Constant.getInstance().paymenttypearraylist.contains(mFilteredOrderArrList.get(pos).getPaymenttype()))
                    {
                        Constant.getInstance().paymenttypearraylist.remove(mFilteredOrderArrList.get(pos).getPaymenttype());
                    ...
                    {
                        Constant.getInstance().paymenttypearraylist.add(mFilteredOrderArrList.get(pos).getPaymenttype());
                        notifyDataSetChanged();
                    }
    ...
    static class ItemBaseViewHolder extends RecyclerView.ViewHolder {
        public ViewPaymenttypeFilterBinding listItemMainBinding;

        public ItemBaseViewHolder(ViewPaymenttypeFilterBinding item) {
            super(item.getRoot());
        ...
        return new Filter() {
            ...
            protected FilterResults performFiltering(CharSequence charSequence) {
                ...
                if (charString.isEmpty()) {
                    ...
                    ArrayList<PaymenttypeModel> filteredList = new ArrayList<>();
                    for (PaymenttypeModel row : mOrderArrList) {
            ...
            protected void publishResults(CharSequence charSequence, FilterResults filterResults) {
                mFilteredOrderArrList = (ArrayList<PaymenttypeModel>) filterResults.values;
                notifyDataSetChanged();
