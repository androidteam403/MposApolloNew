...
public class OrdersFragment extends BaseFragment implements OrdersMvpView, MainActivity.UserIneractionListener, MainActivity.OnBackPressedListener {
    ...
    public void onItemClick(CalculatePosTransactionRes item) {
        ...
        orderListReq.setReceiptId(item.getReturnReceiptId());
    ...
    public void onSuccessdetailOrderList(ArrayList<CalculatePosTransactionRes> orderListRes) {
        ...
        if (orderListRes.size() > 0) {
            startActivityForResult(OrderReturnActivity.getStartIntent(getActivity(), orderListRes.get(0)), REQUEST_CODE);
            getActivity().overridePendingTransition(R.anim.slide_from_right, R.anim.slide_to_left);
        }
    }
