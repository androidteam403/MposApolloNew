...
import com.apollopharmacy.mpospharmacistTest.databinding.ActivityCorporateDetailsBinding;
import com.apollopharmacy.mpospharmacistTest.ui.base.BaseActivity;
import com.apollopharmacy.mpospharmacistTest.ui.corporatedetails.adapter.CorporateDetailAdapter;
import com.apollopharmacy.mpospharmacistTest.ui.corporatedetails.model.CorporateModel;
import com.apollopharmacy.mpospharmacistTest.ui.corporatedetails.model.DropDownLine;
...
public class CorporateDetailsActivity extends BaseActivity implements CorporateDetailsMvpView {
    ...
    CorporateModel.DropdownValueBean tempitem;
    boolean corporatevalue = false;
    ...
    protected void setUp() {
        ...
        //showMessage("this is Corporate List");
    ...
    public void NavigatetoMainScreen() {
       /* if (Constant.getInstance().onlinecorporate) {
            showMessage("Invalid Corporate Id");

        } else {

            Intent returnIntent = new Intent();
            Bundle bundle = new Bundle();
            bundle.putSerializable("corporate_info", tempitem);
    ...
    public void SuccessOnlineorderCorporatelist(GetOnlineCorporateListApiResponse response) {
        if (response.getDropDownLineList() != null) {
            List<DropDownLine> line = response.getDropDownLineList();

            for (DropDownLine lane1 : line) {
                Log.i("tempitem code-->",tempitem.getCode());
                Log.i("lane1 code-->",tempitem.getCode());

                if (tempitem.getCode().equalsIgnoreCase(lane1.getValues())) {
                    Constant.getInstance().onlinecorporate = true;
                     break;
                }
            }
            if (Constant.getInstance().onlinecorporate) {
           ...
           // mPresenter.onlineordercorporate();
        } else {
            Intent returnIntent = new Intent();
            Bundle bundle = new Bundle();
            bundle.putSerializable("corporate_info", tempitem);
    ...
    public void onClickCorporateItem(CorporateModel.DropdownValueBean item) {
        ...
        request.setType("GETONLINEORDERTTRANCKINGREF");
        request.setValue(item.getCode());
        request.setISFMCGReturn(false);
        request.setISPharmaReturn(false);
        mPresenter.getOnlineOrderCorporateList(request);
        //mPresenter.onlineordercorporate();
